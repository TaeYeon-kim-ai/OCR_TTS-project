...
Apart from combining CNN and RNN, it also illustrates how you can instantiate a new layer and use it as an "Endpoint layer" for implementing CTC loss.
 For a detailed guide to layer subclassing, please check out this page in the developer guides.
'''
...
'''
데이터 세트에는 1040 개의 captcha 파일이 png 이미지로 포함되어 있습니다.
각 샘플의 레이블은 파일 이름 인 문자열입니다.
(파일 확장자 빼기) 모델 학습을 위해 문자열의 각 문자를 정수로 매핑합니다. 유사하게 모델의 예측을 문자열로 다시 매핑해야합니다.이를 위해 두 개의 사전을 유지합니다.
문자를 정수로, 정수를 문자로 각각 매핑합니다.
'''
# Path to the data directory
data_dir = Path("F:/BSW_dtaset/data2_parksunwoo/basic_rename/")
0
...
# 컨볼 루션 블록에 의해.
# 컨볼 루션 블록과 각 블록은
# 2 배로 피처를 다운 샘플링하는 풀링 레이어
# 따라서 총 다운 샘플링 계수는 4가됩니다.
downsample_factor = 4
