...
'''
#loop over the output positive and negative directiries
for dirPath in (config.POSITVE_PATH, config.NEGATIVE_PATH):
	if not os.path.exists(dirPath):
    		os.makedirs(dirPath)
...
#경로 내 이미지 루프돌리기
# for (i, imagePath) in enumerate(imagePaths) :
#     	#show a prog] processing imageress report 
# 	print("[INFO] processing image {}/{}...".format(i+1, len(imagePath))) # 이미지 불러오는 정도 표기

# 	#extract the file from the file path and use it to derive the path to the XML annotation file
# 	filename = imagePath.split(os.path.sep)[-1]
# 	filename = filename[:filename.rfind(".")]
# 	annotPath = os.path.sep.join([config.ORIG_ANNOTS, "{}.xml".format(filename)]) #filename의 경로 안에 xml파일을 로드하여 annotPath 변수에 저장한다.

# 	#load the annotation file, build the soup, and initialize our list of ground-truth bounding boxes
# 	contents = open(annotPath).read() #annotPath를 연다.
# 	soup = BeautifulSoup(contents, "html.parser")
# 	gtBoxes = [] #데이터셋의 경계상자(?)넣기?? 일딴 뒤에 가봐야 할 듯

# 	#extract the image dimensions #파싱을 위한 소스
# 	w = int(soup.find("width").string) #넓이 xml주석 파일에서 추출한 이미지 넓이 #파일에서 width라는 스트링으로 표현된 글자안에 값을 가져와라
# 	h = int(soup.find("height").string) #높이 xml주석 파일에서 추출한 이미지 넓이 #파일에서 height라는 str글자 안의 수치를 int로 가져와라
...
for (i, imagePath) in enumerate(imagePaths):
	...
	#IOU
	positiveROIs = 0 #카운트 초기화
	negativeROIs = 0
	
	for proposedRect in proposedRects [:config.MAX_PROPOSALS] : #프로포잘 잡은거 전체 for
		(propStartX, propStartY, propEndX, propEndY) = proposedRect #proposedRect 정의
		
		for gtBox in gtBoxes : #real 바운딩박스 반복해서 가져오기
			iou = compute_iou(gtBox, proposedRect) #iou계산 iou.py에 있음
			(gtStartX, gtStartY, gtEndX, gtEndY) = gtBox
			...
			if iou > 0.7 and positiveROIs <= config.MAX_POSITIVE:
				...
				if not fullOverlap and iou < 0.05 and negativeROIs <= config.MAX_NEGATIVE:
					...
					roi = image[propStartY:propEndY, propStartX:propStartX]
					filename = "{}.png".format(totalNegative)
